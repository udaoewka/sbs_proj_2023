    ...
    <div class="page-menu mt-5 flex justify-center">
	    <div class="btn-group">
	    	...
	    	<c:if test="${startPage > 1 }">
	    		<a class="btn bnt-sm" href="?page=1&boardId=${boardId }">1</a>
	    		<c:if test="${startPage > 2 }">
	    			<a class="btn bnt-sm btn-disabled">...</a>
	    		</c:if>
	    	</c:if>
			<c:forEach begin="${startPage }" end="${endPage }" var="i">
				<a class="btn bnt-sm ${page == i ? 'btn-active' : '' }" href="?page=${i }&boardId=${boardId }">${i }</a>
			</c:forEach>
			<c:if test="${endPage < pagesCount }">
				<c:if test="${endPage < pagesCount - 1}">
					<a class="btn bnt-sm btn-disabled">...</a>
				</c:if>
	    		<a class="btn bnt-sm" href="?page=${pagesCount }&boardId=${boardId }">${pagesCount }</a>
	    	</c:if>
		</div>
	</div>
  </div>
